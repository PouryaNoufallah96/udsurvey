...
interface ISurveyQuestionDefinitionList extends ICustomList {
    id: number;
    title: string;
    category: string;
}
...
const currentSurveyQuestionDefinition = ref<number | null>(null);

const columns = ['تیتر', 'کتگوری', 'وضعیت', ''];
const propsToSearch: (keyof ISurveyQuestionDefinitionList)[] = ['title', 'category'];

const { data: surveyQuestionDefinitions, error, refresh } = await useAsyncData<ISurveyQuestionDefinitionList[]>('get-survey-question-definitions-by-admin', async () => {
    var response = await useBaseFetch<IApiResponse<ISurveyQuestionDefinitionList[]>>('/admin/survey-question-definition/get-survey-question-definitions');
...
const submitDelete = async () => {
    try {
        await useBaseFetch<IApiResponse<undefined>>(`/admin/survey-question-definition/delete-survey-question-definition/${currentSurveyQuestionDefinition.value}`, { method: 'DELETE' })
        closeDeleteModal();
        refresh();
    }
    catch (err) {
        ...
        <NuxtLayout name="dashboard">
            ...
            <template #header>
                <div class="flex justify-between">
                    <h2 class="text-header">سوالات نظرسنجی‌</h2>
                    <BaseButton :btn-type="ButtonType.SUCCESS" text="سوال جدید" @click="addSurveyQuestionDefinition">
                    </BaseButton>
                </div>
            </template>

            <template v-if="!error">
                <BaseTable :props-to-search="propsToSearch" :columns="columns" :data="surveyQuestionDefinitions"
                    v-if="surveyQuestionDefinitions && surveyQuestionDefinitions.length">
                    <template v-slot:normal="{ row }">
                        ...
                        <td class="tbl-row ">
                            <div class="flex gap-3">
                                <nuxt-link :to="`/admin/survey/question-definition/update/${row.id}`"><i
